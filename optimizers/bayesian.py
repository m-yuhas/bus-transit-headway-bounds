...
class BayesianOptimizer():
    ...
    def _get_expected_improvement(self, x_new, gpr_idx):
        ...
        mean_y_new, sigma_y_new = self.gauss_prs[gpr_idx].predict(x_new.reshape(1,-1), return_std=True)
        ...
        #max_mean_y = numpy.max(mean_y)
        mean_y = sum(self.y_prev)/len(self.y_prev)
        max_mean_y = max(self.y_prev)
    ...
    def _get_probability_improvement(self, x_new, gpr_idx):
        mean_y_new, sigma_y_new = self.gauss_prs[gpr_idx].predict(numpy.array([x_new]), return_std=True)
        sigma_y_new = sigma_y_new.reshape(-1, 1)
        if sigma_y_new == 0.0:
            return 0.0
        ...
        mean_c_new, sigma_c_new = self.constraint_pr.predict(numpy.array([x_new]), return_std=True)
        pf = norm.cdf(self.constraint, loc=mean_c_new, scale=sigma_c_new)
        return pi
        #return pf * pi
    ...
    def _initialize(self):
        ...
        for n in range(self.n_init):
            ...
            for var, support in self.variables.items():
                if isinstance(support, tuple):
                    x[var] = random.uniform(*support)
                elif isinstance(support, set):
                    x[var] = random.choice(list(support))
    ...
    def _get_next_probable_point(self, x, y, grp_id):
        ...
        x_optimal = None
        for x_start in [numpy.random.uniform(-1, 1, x.shape[1]) for _ in range(self.batch_size)]:
    ...
    def _hist_to_numpy(self) -> tuple[list[numpy.array], list[numpy.array], list[numpy.array]]:
        ...
        for cat in itertools.product(*[v for v in self.categorical_vars.values()]):
            x_cat = []
            y_cat = []
            for prevx, prevy in zip(self.x_prev, self.y_prev):
    ...
    def _x_next_to_dict(self, x_next, cat):
        print(x_next)
        print(cat)
        cat_vals = list(itertools.product(*[v for v in self.categorical_vars.values()]))[cat]
        x = {}
        for idx, var in enumerate(self.categorical_vars.keys()):
            x[var] = cat_vals[idx]
        for idx, item in enumerate(self.ordinal_vars.items()):
            k, v = item
            x[k] = min(max(x_next[idx], v[0]), v[1])
        return x

    def _get_random_x(self, cat):
        cat_vals = list(itertools.product(*[v for v in self.categorical_vars.values()]))[cat]
        x = {}
        for idx, var in enumerate(self.categorical_vars.keys()):
            x[var] = cat_vals[idx]
        ...
        for i in range(self.n_iter):
            ...
            for cat, data in enumerate(zip(x, y)):
                ...
                if len(x_cat) > 0:
                    ...
                    #y_scaler.fit(y_cat)
                    self.gauss_prs[cat].fit(x_scaler.transform(x_cat), y_cat.reshape(-1, 1))
                    #self.constraint_pr.fit(scaler.transform(self.x_init), self.c_init <= self.constraint)
                    x_next, ei = self._get_next_probable_point(x_scaler.transform(x_cat), y_cat.reshape(-1, 1), cat)
                    x_next = x_scaler.inverse_transform(x_next.reshape(1, -1))[0]
            ...
            override_stopping = True if max(ucbs) == float('inf') else False
            ...
            if self.y_prev[-1] > y_max: #and c_next <= self.constraint:
                y_max = self.y_prev[-1]
                optimal_x = self.x_prev[-1]
                #optimal_ei = ei
                n_iter_no_improvement = 0

            if n_iter_no_improvement > self.early_stopping and self.early_stopping > 0 and not override_stopping:
                break
